...
Begin VB.Form FrmTipoDocumentos 
   ...
   ClientTop       =   345
   ClientWidth     =   7680
   Icon            =   "FrmTipoDocumentos.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   3060
   ScaleWidth      =   7680
   ...
   Begin VB.CommandButton cmdprimero 
      ...
      Height          =   615
      Left            =   2760
      Picture         =   "FrmTipoDocumentos.frx":08CA
      Style           =   1  'Graphical
      TabIndex        =   5
      ToolTipText     =   "Primero"
      Top             =   1755
   ...
   Begin VB.CommandButton cmdultimo 
      ...
      Height          =   615
      Left            =   4665
      Picture         =   "FrmTipoDocumentos.frx":0BD4
      Style           =   1  'Graphical
      TabIndex        =   4
      ToolTipText     =   "Ultimo"
      Top             =   1755
   ...
   Begin VB.CommandButton cmdsiguiente 
      ...
      Height          =   615
      Left            =   4050
      Picture         =   "FrmTipoDocumentos.frx":0EDE
      Style           =   1  'Graphical
      TabIndex        =   3
   ...
   Begin VB.CommandButton cmdanterior 
      ...
      Height          =   615
      Left            =   3375
      Picture         =   "FrmTipoDocumentos.frx":11E8
      Style           =   1  'Graphical
      TabIndex        =   2
...
Dim codigo As Long
    ...
    Else
        If Ope = "A" Then
            rs.Open "Select max(codigo) as cod from TipoDocumentos", DataEnvironment1.Sistema, adOpenStatic, adLockReadOnly
            ...
            DataEnvironment1.dbo_TIPODOCUMENTO "A", codigo, Trim(txtDescripcion), fecha, UsuarioSistema!codigo, 0, 0
            MsgBox "La Operación se ha realizado con éxito", 48, "Atencion"
        Else
            If Ope = "M" Then
                fecha = Month(Date) & "/" & Day(Date) & "/" & Year(Date)
                DataEnvironment1.dbo_TIPODOCUMENTO "M", Val(txtCodigo), Trim(txtDescripcion), 0, 0, 0, 0
                DataEnvironment1.dbo_GRABARBITACORA Trim(txtCodigo), "TipoDocumentos", UsuarioSistema!codigo, fecha, Time, "M"
                MsgBox "La Operación se ha realizado con éxito", 48, "Atencion"
            End If
        End If
    End If
...
Private Sub cmdanterior_Click()
    rsDoc.MovePrevious
    If Not rsDoc.BOF Then
        txtCodigo = rsDoc!codigo
        txtDescripcion = rsDoc!DESCRIPCION
...
'
'    FrmHelp.Show
'    CargarHelp "TipoDocumentos", "Codigo", "Descripcion", "codigo", "descripcion"
'    FrmHelp.Tag = Me.Name
    Dim resu As String
    resu = frmBuscar.MostrarCodigoDescripcionActivo("TipoDocumentos")
    If resu > "" Then
        txtCodigo = resu
        CargarDatos
        Call HabilitoControles(True, False, True, False, True, False)
...
Dim mensaje As String
    ...
    If mensaje = 6 Then
        fecha = Month(Date) & "/" & Day(Date) & "/" & Year(Date)
        DataEnvironment1.dbo_TIPODOCUMENTO "B", Trim(txtCodigo), "", 0, 0, UsuarioSistema!codigo, fecha
        DataEnvironment1.dbo_GRABARBITACORA Trim(txtCodigo), "TipoDocumentos", UsuarioSistema!codigo, fecha, Time, "B"
...
Dim rsDoc As New ADODB.Recordset
    ...
    Ope = "A"
    LimpioTxt
    rsDoc.Open "select max(codigo) as cod from tipodocumentos", DataEnvironment1.Sistema, adOpenStatic, adLockReadOnly
    If Not IsNull(rsDoc!cod) Then
        txtCodigo = rsDoc!cod + 1
    Else
        txtCodigo = 1
    End If
    rsDoc.Close
...
Sub CargarDatos()
    ...
    If rsDoc.State = 1 Then
        ...
        Set rsDoc = Nothing
    End If
    rsDoc.Open "select * from tipodocumentos where activo=1", DataEnvironment1.Sistema, adOpenDynamic, adLockOptimistic
    
    If Not rsDoc.EOF Then
        rsDoc.MoveFirst
        rsDoc.Find "Codigo= " & STR(Trim(txtCodigo))
        txtDescripcion = rsDoc!DESCRIPCION
        Call HabilitoBotonesMoverse(True, True, True, True)
    End If
