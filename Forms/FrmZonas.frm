...
Dim codigo As Long
    ...
    Else
        If Ope = "A" Then
            rs.Open "Select max(codigo) as cod from Zonas", DataEnvironment1.Sistema, adOpenStatic, adLockReadOnly
            If Not rs.EOF Then
                codigo = rs!cod + 1
            Else
                codigo = 1
            ...
            DataEnvironment1.dbo_ZONA "A", codigo, Trim(txtDescripcion), fecha, UsuarioSistema!codigo, 0, 0
            MsgBox "La Operación se ha realizado con éxito", 48, "Atencion"
        Else
            If Ope = "M" Then
                fecha = Month(Date) & "/" & Day(Date) & "/" & Year(Date)
                DataEnvironment1.dbo_ZONA "M", Val(txtCodigo), Trim(txtDescripcion), 0, 0, 0, 0
                DataEnvironment1.dbo_GRABARBITACORA Trim(txtCodigo), "Zonas", UsuarioSistema!codigo, fecha, Time, "M"
                MsgBox "La Operación se ha realizado con éxito", 48, "Atencion"
            End If
        End If
    End If
...
Private Sub cmdanterior_Click()
    rsZona.MovePrevious
    If Not rsZona.BOF Then
        txtCodigo = rsZona!codigo
        txtDescripcion = rsZona!DESCRIPCION
...
'
'    FrmHelp.Show
'    CargarHelp "Zonas", "Codigo", "Descripcion", "codigo", "descripcion"
'    FrmHelp.Tag = Me.Name

    Dim resu As String
    resu = frmBuscar.MostrarCodigoDescripcionActivo("Zonas")
    If resu > "" Then
        txtCodigo = resu
        CargarDatos
        Call HabilitoControles(True, False, True, False, True, False)
...
Dim mensaje As String
    ...
    If mensaje = 6 Then
        fecha = Month(Date) & "/" & Day(Date) & "/" & Year(Date)
        DataEnvironment1.dbo_ZONA "B", Trim(txtCodigo), "", 0, 0, UsuarioSistema!codigo, fecha
        DataEnvironment1.dbo_GRABARBITACORA Trim(txtCodigo), "Zonas", UsuarioSistema!codigo, fecha, Time, "B"
...
Dim rsZona As New ADODB.Recordset
    ...
    Ope = "A"
    LimpioTxt
    rsZona.Open "select max(codigo) as cod from Zonas", DataEnvironment1.Sistema, adOpenStatic, adLockReadOnly
    If Not IsNull(rsZona!cod) Then
        txtCodigo = rsZona!cod + 1
    Else
        txtCodigo = 1
    End If
    rsZona.Close
...
Sub CargarDatos()
    ...
    If rsZona.State = 1 Then
        ...
        Set rsZona = Nothing
    End If
    rsZona.Open "select * from zonas where activo=1", DataEnvironment1.Sistema, adOpenDynamic, adLockOptimistic
    
    If Not rsZona.EOF Then
        rsZona.MoveFirst
        rsZona.Find "Codigo= " & STR(Trim(txtCodigo))
        txtDescripcion = rsZona!DESCRIPCION
        Call HabilitoBotonesMoverse(True, True, True, True)
    End If
